package com.xtan.component.dialog.other
...
import com.xtan.component.dialog.extensions.UtilsExtension.Companion.unDisplayViewSize
...
open class DialogOptions() : Parcelable {
     ...
     */
    @LayoutRes
    open var layoutId = R.layout.dialog_loading


    /**
     * 是否启用防止内存泄露模式
     * 当在activity中使用CustomDialog时，如果不是一次性使用，而是被一个对象引用，多次使用
     ...
     */
    var dialogThemeFun: (customDialog: CustomDialog) -> Int = {
        it.dialogActivity!!.run {
            ...
            if (this.window.decorView.systemUiVisibility == View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
                ...
                R.style.CustomDialogFullScreen
     ...
     * dataBinding绑定
     */
    var bindingListener: ((container: ViewGroup?, dialog: CustomDialog) -> View)? = null
     ...
     * 该方法对象可重写
     */
    var setStatusBarModeFun: (customDialog: CustomDialog) -> Unit = { CustomDialog ->
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {//5.0 全透明实现
            CustomDialog.dialogActivity.apply {
                //如果activity是否是占满全屏并且依旧保留状态栏（沉浸式状态栏）
                if (this!!.window.decorView.systemUiVisibility == View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
                        || this.window.decorView.systemUiVisibility == View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN or View.SYSTEM_UI_FLAG_LAYOUT_STABLE
                        || this.window.decorView.systemUiVisibility == View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN or View.SYSTEM_UI_FLAG_LAYOUT_STABLE or View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR) {
                    ...
                    (findViewById<View>(android.R.id.content) as ViewGroup).getChildAt(0).fitsSystemWindows.let {
                        //（与activity保持一致）
                        CustomDialog.dialog?.window?.decorView?.fitsSystemWindows = it
                        if (it) {//无论是否预留了statusBar，都将dialog设置可占满全屏
                            CustomDialog.dialog?.window?.decorView?.systemUiVisibility = View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN or View.SYSTEM_UI_FLAG_LAYOUT_STABLE
                            //此时可以设置dialogStatusBarColor
                            //CustomDialog.dialog.window.statusBarColor = dialogStatusBarColor
                        } else {//如果不预留statusrBar，那么dialog设置可占满全屏
                            CustomDialog.dialog?.window?.decorView?.systemUiVisibility = this.window.decorView.systemUiVisibility
                        }
                    }
                    //CustomDialog.dialog.window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)
                } else {//非沉浸式状态栏（包括普通的带状态栏页面以及全屏无状态栏页面）此时的statusBarColor不生效
                    CustomDialog.dialog?.window?.decorView?.systemUiVisibility = View.SYSTEM_UI_FLAG_VISIBLE
                    //CustomDialog.dialog.window.statusBarColor = dialogStatusBarColor
                }
            }
        } else {//4.4 全透明状态栏
            CustomDialog.dialog?.window?.addFlags(CustomDialog.dialogActivity!!.window.attributes.flags)
        }
    }
     ...
     */
    fun setDialogTheme(function: (customDialog: CustomDialog) -> Int) {
     ...
     */
    fun setStatusMode(function: (customDialog: CustomDialog) -> Unit) {
